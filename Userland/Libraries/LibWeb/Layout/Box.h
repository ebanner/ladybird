...
public:
    ...
    void add_contained_child(Box& child) { m_contained_children.append(child); }
    void clear_contained_children() { m_contained_children.clear(); }
    Vector<JS::NonnullGCPtr<Box>> const& contained_children() const { return m_contained_children; }
...
private:
    ...
    Vector<JS::NonnullGCPtr<Box>> m_contained_children;
